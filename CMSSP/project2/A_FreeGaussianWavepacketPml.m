...
% a dirt
%
% For the boundaries, periodic boundary conditions are assumed. 
% Additionally, an absorbing Layer, modelled with PMl, is added in
% x-direction to simulate absorbing boundary conditions.
%
% NATURAL UNITS:
% units where: 
...
x_pml = 0.5;
idx_r = xx > x_pml; 
sigma(idx_r) = sigma_0 .* (xx(idx_r)-x_pml).^2;

idx_l = xx < -x_pml;
sigma(idx_l) = sigma_0 .* (xx(idx_l)+x_pml).^2;
...
[sol_u, sol_v] = diracEq2D.solveEquationPml(...
    u_init, ...
    v_init, ...
    xx, ...
    yy, ...
